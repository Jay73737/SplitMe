...
if (Test-Path $requirementsFile) {
    ...
    & $pipExe install torch==2.6.0 torchvision==0.21.0 torchaudio==2.6.0 --index-url https://download.pytorch.org/whl/cu118
...
if (Test-Path (Join-Path $ffmpegDirectory "ffmpeg.exe")) {
    ...
    git clone https://github.com/adefossez/demucs.git -Wait
...
if (Command-Exists "nvcc") {
    ...
    Write-Host "nvcc not found. Installing CUDA Toolkit $cudaVersion..."
    Start-BitsTransfer -Source $cudaDownloadUrl -Destination $cudaInstallerPath -Wait
    Start-Process -FilePath $cudaInstallerPath -ArgumentList "-s", "-loglevel=error" -Wait
