...
class MainGUI(QWidget):  
    ...
    def __init__(self):
        ...
        self.search_button.clicked.connect(self.open_browser)
        layout.addWidget(self.search_button)

        self.search_soundcloud_button = QPushButton("Search on Soundcloud")
        self.search_soundcloud_button.clicked.connect(lambda: self.open_browser(True))
        ...
        self.url_input.returnPressed.connect(self.open_browser)
        ...
        self.current_file_path = os.path.abspath(__file__)  # Absolute path of the script
        self.current_dir = os.path.dirname(self.current_file_path)
        self.save_location = os.path.join(self.current_dir, "Downloads")        
        self.save_button.clicked.connect(self.select_save_location)
        layout.addWidget(self.save_button)

        self.save_path = ""
        self.save_label = QLabel(f"Save Location: {self.save_location}")
    ...
    def open_browser(self, soundcloud=False):
        try:
            if self.ytd.isRunning():
                return
        except:
            pass
        ...
        if ".com" in self.url_input.text():
            if soundcloud:
                self.ytd = YoutubeDownloader.YoutubeDownloader(f"https://soundcloud.com?q={query.replace(' ', '+')}", True)
            else:
                self.ytd = YoutubeDownloader.YoutubeDownloader(f"https://youtube.com?q={query.text().replace(' ', '+')}", False)
        else:
            
            if soundcloud:
                self.ytd = YoutubeDownloader.YoutubeDownloader(query, True)
            else:
                self.ytd = YoutubeDownloader.YoutubeDownloader(query, False)                  
        self.ytd.finished.connect(self.set_url)
        self.ytd.start()
