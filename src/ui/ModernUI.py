...
if not os.path.exists(config_path):
    ...
    with open(config_path, "w", encoding="utf-8") as _f:
        json.dump({"api_key": key}, _f, indent=2)
YOUTUBE_API_KEY = ""
try:
...
class ResultCard(QFrame):
    ...
    def _ensure_audio(self):
        """Download YouTube audio as .mp3 via yt‑dlp on first drag."""
    ...
    def mouseMoveEvent(self, e):
        ...
        # grab pixmap of the whole card for drag cursor
        pixmap = self.grab()
        mime = QMimeData()
        ...
        # drop animation back into place
        self._drop_back()

    # ----- lift / drop animations during drag -----
    def _lift(self):
        """Animate the card upward slightly to show it's picked up."""
        if self._lifted:
            return
        self._lifted = True
        end_pos = self.pos() - QPoint(0, 15)
    ...
    def _drop_back(self):
        """Return the card to its original grid position after drag."""
        if not self._lifted:
            return
        self._lifted = False
        anim = QPropertyAnimation(self, b"pos", self)
        anim.setDuration(120)
        anim.setStartValue(self.pos())
        anim.setEndValue(self._original_pos)
        anim.setEasingCurve(QEasingCurve.Type.OutCubic)
        anim.start(QPropertyAnimation.DeletionPolicy.DeleteWhenStopped)
    ...
    def paintEvent(self, ev):
        ...
        # inner rounded rect (card already has radius 10 background)
...
class SearchResults(QFrame):
    def __init__(self, parent=None):
        ...
        # ensure the viewport and content are transparent
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.btn_min.setIcon(QIcon("assets/MINIMIZE.svg"))
        ...
        self.btn_close.setIcon(QIcon("assets/CLOSE.svg"))
