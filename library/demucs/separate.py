...
def get_parser():
    ...
    parser.add_argument("-r", "--remove-model-dir",
                        action="store_true",
                        help="Removes the model directory name from the output path directory. "
                             "Used this so I could make the separate outputs from different models"
                             "store in the song folder eg. /the Beatles - Something/htdemucs/track.wav  " 
                             "rather than htdemucs/the Beatles - Something/htdemucs/track.wav")
    ...
    for track in args.tracks:
        ...
        if args.stem is None:
            for source, name in zip(sources, model.sources):
                file_path = args.filename.format(track=track.name.rsplit(".", 1)[0],
